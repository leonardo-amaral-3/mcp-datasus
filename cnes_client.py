"""
Cliente CNES — lê dados operacionais de estabelecimentos do MinIO (Parquet).

Carrega leitos, serviços, habilitações e profissionais na primeira chamada.
"""
...
def _agrupar_por_cnes(tabela, colunas_extra: list[str]) -> dict[str, list[dict]]:
    """Agrupa linhas de uma tabela pyarrow pelo campo 'cnes'."""
...
def _carregar(competencia: str | None = None):
    ...
    if not comp:
        raise RuntimeError("Nenhuma competência CNES encontrada no MinIO.")
...
def consultar_cnes(codigo: str) -> dict | None:
    """Retorna dados agregados de um CNES (leitos, serviços, habilitações, profissionais)."""
    _carregar()
    codigo = codigo.strip()
    ...
    # Agrupar profissionais por ocupação
...
def buscar_profissionais(cnes: str, co_ocupacao: str = "") -> list[dict]:
    """Lista profissionais de um CNES, com filtro opcional por ocupação."""
    _carregar()
...
def info() -> dict:
    """Retorna metadata do CNES carregado."""
    _carregar()
    return {
        "competencia": _competencia,
